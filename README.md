...
Создать произвольное количество треков:

```bash
php artisan tracks:seed 100
```

Связи (артисты, альбомы, жанры) подбираются случайно. Сопутствующие сущности создадутся автоматически, если их ещё нет.
